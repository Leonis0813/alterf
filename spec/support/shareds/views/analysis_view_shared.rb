...
module AnalysisViewShared
  include CommonViewShared
  include AnalysisViewHelper

  DEFAULT_PER_PAGE = 1

  shared_context '分析ジョブを作成する' do |total: DEFAULT_PER_PAGE, update_attribute: {}|
  ...
  shared_examples '画面共通テスト' do |expected: {}|
    ...
    it_behaves_like '表示件数情報が表示されていること',
                    total: expected[:total] || DEFAULT_PER_PAGE,
                    from: expected[:from] || 1,
                    to: expected[:to] || DEFAULT_PER_PAGE
    it_behaves_like 'テーブルが表示されていること',
                    rows: expected[:rows] || DEFAULT_PER_PAGE
  end
