...
describe 'analyses/manage', type: :view do
  ...
  shared_context '分析ジョブを作成する' do |total: per_page, attribute: {}|
    before(:all) do
  ...
  shared_examples 'ジョブが実行中状態になっていること' do
    it do
      ...
      rows.each do |row|
        is_asserted_by { row.xpath('//td')[4].text == '実行中' }
      end
    end
  end
  ...
  shared_examples 'ジョブが完了状態になっていること' do
    it do
      ...
      rows.each do |row|
        is_asserted_by { row.xpath('//td')[4].text == '完了' }
      end
    end
  end
  ...
  context '実行中の場合' do
    ...
    include_context '分析ジョブを作成する', attribute: default_attribute
  ...
  context '完了している場合' do
    ...
    include_context 'トランザクション作成'
    include_context '分析ジョブを作成する', attribute: attribute
    include_context 'HTML初期化'
    it_behaves_like '画面共通テスト'
    it_behaves_like 'ページングボタンが表示されていないこと'
  ...
  context "分析ジョブ情報が#{per_page * (Kaminari.config.window + 2)}件の場合" do
    ...
    include_context 'トランザクション作成'
    include_context '分析ジョブを作成する', total: total, attribute: default_attribute
    include_context 'HTML初期化'
