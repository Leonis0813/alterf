...
class AnalysisMailer < ApplicationMailer
  ...
  def finished(analysis, is_success)
    ...
    attachments['training_data.yml'] = File.read(File.join(Rails.root, "tmp/files/#{analysis_id}/analysis.yml"))
    attachments['model.rf'] = File.read(File.join(Rails.root, "tmp/files/#{analysis_id}/model.rf"))
